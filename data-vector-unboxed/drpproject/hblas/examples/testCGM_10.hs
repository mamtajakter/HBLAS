...
main = do putStr "\n\nCGM 10:\n"
          ...
          putStrLn $ "Discard Test : " ++ (show ((  (diffUTCTime t10 t0)) /8))
          ...
          t0 <- getCurrentTime
          t1 <- seq (conjugateGradient (2^10) a10 vecX10 vecY10) getCurrentTime
          t2 <- seq (conjugateGradient (2^10) a10 vecX10 vecY10) getCurrentTime
          t3 <- seq (conjugateGradient (2^10) a10 vecX10 vecY10) getCurrentTime
          putStrLn $ (show  (((diffUTCTime t3 t2) + (diffUTCTime t2 t1) +(diffUTCTime t1 t0)) /r))          
          -- t0 <- getCurrentTime
