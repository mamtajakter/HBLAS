...
main = do putStr "\n\nCGM 12:\n"
          ...
          putStrLn $ "Discard Test : " ++ (show ((  (diffUTCTime t12 t0)) /8))
          ...
          -- putStrLn $ (show  (((diffUTCTime t3 t2) + (diffUTCTime t2 t1) +(diffUTCTime t1 t0)) /r))          
          t0 <- getCurrentTime
          t1 <- seq (conjugateGradient (2^12) a12 vecX12 vecY12) getCurrentTime
          t2 <- seq (conjugateGradient (2^12) a12 vecX12 vecY12) getCurrentTime
          t3 <- seq (conjugateGradient (2^12) a12 vecX12 vecY12) getCurrentTime
          putStrLn $ (show  (((diffUTCTime t3 t2) + (diffUTCTime t2 t1) +(diffUTCTime t1 t0)) /r))          
