...
main = do putStr "\n\nCGM 11:\n"
          ...
          putStrLn $ "Discard Test : " ++ (show ((  (diffUTCTime t11 t0)) /8))
          ...
          -- putStrLn $ (show  (((diffUTCTime t3 t2) + (diffUTCTime t2 t1) +(diffUTCTime t1 t0)) /r))          
          t0 <- getCurrentTime
          t1 <- seq (conjugateGradient (2^11) a11 vecX11 vecY11) getCurrentTime
          t2 <- seq (conjugateGradient (2^11) a11 vecX11 vecY11) getCurrentTime
          t3 <- seq (conjugateGradient (2^11) a11 vecX11 vecY11) getCurrentTime
          putStrLn $ (show  (((diffUTCTime t3 t2) + (diffUTCTime t2 t1) +(diffUTCTime t1 t0)) /r))          
          -- t0 <- getCurrentTime
