...
-- Could not deduce (Functor V.Vector) arising from a use of ‘fmap’
--       from the context: Num n
--         bound by the type signature for:
--                    scal :: forall n. Num n => n -> V.Vector n -> V.Vector n
--         at haskell/HBLAS/Level1.hs:12:1-49
--       There are instances for similar types:
--         instance Functor Data.Vector.Vector -- Defined in ‘Data.Vector’
--     • In the expression: fmap (alpha *) x
--       In an equation for ‘scal’: scal !alpha !x = fmap (alpha *) x
--    |
-- 13 | scal !alpha !x= fmap (alpha*) x
--    |                 ^^^^^^^^^^^^^^^
{-# INLINE scal #-}
scal :: ( Num n, V.Unbox n) => n -> V.Vector n -> V.Vector n
scal !alpha !x= V.map (alpha*) x

--Done
{-# INLINE axpy #-}
axpy :: ( Num n, V.Unbox n) => n -> V.Vector n -> V.Vector n -> V.Vector n
axpy !alpha !xs !ys =V.zipWith (\x y -> y+alpha*x) xs ys

--Done
{-# INLINE dot #-}
dot :: ( Num n, V.Unbox n) => V.Vector n -> V.Vector n -> n
dot !a !b = V.sum (V.zipWith (*) a b)

{-# INLINE nrm2 #-}
nrm2 :: (  Num n, Floating n, V.Unbox n) => V.Vector n -> n
nrm2 = sqrt . V.sum . V.map (\i -> i * i)

{-# INLINE rot #-}
rot :: (Floating n, Eq n, Num n, V.Unbox n) => V.Vector n -> V.Vector n -> n  -> (V.Vector n, V.Vector n)
rot !x !y !c
         | c==1      = (x,y)
...
--Done
{-# INLINE copy #-}
copy :: ( Eq n, Num n, V.Unbox n) => V.Vector n -> V.Vector n -> V.Vector n
copy xs _ = xs
...
--Done
{-# INLINE asum #-}
asum :: ( Num n,  V.Unbox n) => V.Vector n -> n
asum !y= V.foldr' (\x a -> abs x + a) 0 y

--Done
{-# INLINE idamax #-}
idamax :: (Num n, Ord n,  V.Unbox n)
       => V.Vector n -> Maybe Int
idamax !a = V.elemIndex (V.maximum (V.map abs a)) (V.map abs a)

{-# INLINE idamin #-}
idamin :: (Num n, Ord n, V.Unbox n)
       => V.Vector n -> Maybe Int
idamin !a = V.elemIndex (V.minimum (V.map abs a)) (V.map abs a)

{-# INLINE axpyaxpy #-}
axpyaxpy ::  ( Num n, V.Unbox n) => n -> n -> V.Vector n -> V.Vector n -> V.Vector n
axpyaxpy !a1 !a2 !x !y  = let !t1 = axpy a1 x y in axpy a2 t1 x

{-# INLINE axpyaxpy' #-}
axpyaxpy' ::  ( Num n, V.Unbox n) => n -> n -> V.Vector n -> V.Vector n -> V.Vector n -> V.Vector n
axpyaxpy' !a !b !x !y !z  = let !t = axpy a x y in axpy b t z

{-# INLINE axpy' #-}
axpy' :: ( Num n, V.Unbox n) => n -> V.Vector n -> V.Vector n -> V.Vector n-> V.Vector n
